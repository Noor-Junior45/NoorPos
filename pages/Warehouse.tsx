...
export const Warehouse: React.FC<WarehouseProps> = ({ initialAction, onClearAction }) => {
  ...
  const validateProduct = () => {
    ...
    if (!newProduct.sellPrice || isNaN(newProduct.sellPrice) || newProduct.sellPrice <= 0) newErrors.add('sellPrice');
  ...
  const handleSaveProduct = async () => { 
      if (pendingBulkItems.length > 0) {
          ...
          const hasUnfinishedEntry = newProduct.name?.trim() || (newProduct.sellPrice && newProduct.sellPrice > 0);
        ...
        <Card className="max-w-5xl mx-auto !p-8 shadow-lg">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <div className="md:col-span-2 space-y-2">
                    <label className={`font-bold text-sm flex items-center justify-between transition-colors ${validationErrors.has('name') ? 'text-red-600' : 'text-gray-700'}`}>
                        <span className="flex items-center gap-2"><FileText size={16} className={validationErrors.has('name') ? 'text-red-500' : 'text-blue-500'}/> Product Name *</span>
                        {validationErrors.has('name') && <span className="text-[10px] font-black uppercase animate-pulse">Required!</span>}
                    </label>
                    <Input 
                        ref={editNameRef}
                        ...
                        autoFocus={!isEditing}
                        className={`w-full border-2 transition-all !py-3 !px-6 ${validationErrors.has('name') ? 'border-red-400 bg-red-50/50' : 'border-blue-200 focus:border-blue-500 bg-blue-50/10'}`}
                    />
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <select 
                        ...
                        className="w-full rounded-lg px-6 py-3 text-base bg-blue-50/10 border-2 border-blue-200 text-gray-900 focus:outline-none focus:border-blue-500 transition-all appearance-none h-[52px]"
                    >
                ...
                <div className="space-y-2">
                    <label className={`font-bold text-sm flex items-center justify-between transition-colors ${validationErrors.has('sellPrice') ? 'text-red-600' : 'text-gray-700'}`}>
                        <span className="flex items-center gap-2"><TagIcon size={16} className={validationErrors.has('sellPrice') ? 'text-red-500' : 'text-green-600'}/> Sell Price ({settings.currencySymbol}) *</span>
                        {validationErrors.has('sellPrice') && <span className="text-[10px] font-black uppercase animate-pulse">Required!</span>}
                    </label>
                    <Input 
                        ref={editSellRef}
                        ...
                        onChange={e => {
                            setNewProduct({...newProduct, sellPrice: parseFloat(e.target.value) || 0});
                            if (validationErrors.has('sellPrice')) {
                                setValidationErrors(prev => { const next = new Set(prev); next.delete('sellPrice'); return next; });
                            }
                        }}
                        className={`w-full border-2 transition-all !py-3 !px-6 ${validationErrors.has('sellPrice') ? 'border-red-400 bg-red-50/50' : 'border-blue-200 focus:border-blue-500 bg-green-50/10'}`}
                    />
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex w-full border-2 border-purple-200 rounded-lg overflow-hidden focus-within:border-purple-500 bg-purple-50/10 transition-colors h-[52px] relative">
                        ...
                        <div className="flex items-center gap-1 bg-purple-50/80 px-2 border-l border-purple-100 h-full shrink-0">
                            ...
                            <input type="number" placeholder="Box" className="w-14 text-center bg-white border border-purple-200 rounded text-xs py-1.5 focus:border-purple-500 outline-none" value={batchConfig.packs} onChange={(e) => handleBatchChange('packs', e.target.value)} />
                            <span className="text-gray-400 font-bold text-xs">×</span>
                            <input type="number" placeholder="Qty" className="w-14 text-center bg-white border border-purple-200 rounded text-xs py-1.5 focus:border-purple-500 outline-none" value={batchConfig.perPack} onChange={(e) => handleBatchChange('perPack', e.target.value)} />
                        </div>
                    </div>
                </div>
             ...
             <div className="mt-8 border-t border-gray-100 pt-4 flex justify-center">
                 <button onClick={() => setShowMoreFields(!showMoreFields)} className="flex items-center gap-2 text-blue-600 font-bold text-sm bg-blue-50 px-4 py-2 rounded-full hover:bg-blue-100 transition-colors">
             ...
             {showMoreFields && (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mt-6 animate-in slide-in-from-top-4 fade-in duration-300">
                    <div className="space-y-2">
                        <label className="font-bold text-gray-700 text-sm flex items-center gap-2"><Factory size={16} className="text-green-600"/> Buy Price ({settings.currencySymbol})</label>
                        <Input ref={editBuyRef} onKeyDown={(e) => handleEditorKeyDown(e, editWholesaleRef)} type="number" placeholder="0.00" value={newProduct.buyPrice || ''} onChange={e => setNewProduct({...newProduct, buyPrice: parseFloat(e.target.value) || 0})} className="w-full border-2 border-green-200 focus:border-green-500 bg-green-50/10 rounded-lg !py-3 !px-4" />
                    </div>
                    <div className="space-y-2">
                        <label className="font-bold text-gray-700 text-sm flex items-center gap-2"><Box size={16} className="text-green-600"/> Wholesale Price ({settings.currencySymbol})</label>
                        <Input ref={editWholesaleRef} onKeyDown={(e) => handleEditorKeyDown(e, editTaxRef)} type="number" placeholder="0.00" value={newProduct.wholesalePrice || ''} onChange={e => setNewProduct({...newProduct, wholesalePrice: parseFloat(e.target.value) || 0})} className="w-full border-2 border-green-200 focus:border-green-500 bg-green-50/10 rounded-lg !py-3 !px-4" />
                    </div>
                    <div className="space-y-2">
                        <label className="font-bold text-gray-700 text-sm flex items-center gap-2"><Percent size={16} className="text-green-600"/> Tax Rate (%)</label>
                        <Input ref={editTaxRef} onKeyDown={(e) => handleEditorKeyDown(e, editLocationRef)} type="number" placeholder="0" value={newProduct.taxRate || ''} onChange={e => setNewProduct({...newProduct, taxRate: parseFloat(e.target.value) || 0})} className="w-full border-2 border-green-200 focus:border-green-500 bg-green-50/10 rounded-lg !py-3 !px-4" />
                    </div>
                    <div className="space-y-2">
                        <label className="font-bold text-gray-700 text-sm flex items-center gap-2"><MapPin size={16} className="text-purple-500"/> Location</label>
                        <Input ref={editLocationRef} onKeyDown={(e) => handleEditorKeyDown(e, editUnitSizeRef)} placeholder="e.g. Aisle 3" value={newProduct.location} onChange={e => setNewProduct({...newProduct, location: e.target.value})} className="w-full border-2 border-purple-200 focus:border-purple-500 bg-purple-50/10 rounded-lg !py-3 !px-4" />
                    </div>
                    <div className="space-y-2">
                        <label className="font-bold text-gray-700 text-sm flex items-center gap-2"><Scale size={16} className="text-purple-500"/> Unit Size</label>
                        <div className="flex w-full shadow-sm rounded-lg overflow-hidden border-2 border-purple-200 focus-within:border-purple-500 transition-all bg-purple-50/10 h-[52px]">
                            <input ref={editUnitSizeRef} onKeyDown={(e) => handleEditorKeyDown(e, editLowStockRef)} type="text" placeholder="1" className="w-1/3 bg-transparent px-3 text-center font-medium focus:outline-none border-r border-purple-200 text-gray-700 h-full" value={newProduct.capacity || ''} onChange={(e) => setNewProduct({...newProduct, capacity: e.target.value})} />
                            <select value={newProduct.unit || 'pcs'} onChange={e => setNewProduct({...newProduct, unit: e.target.value})} className="w-2/3 bg-transparent px-3 font-medium text-gray-700 focus:outline-none cursor-pointer hover:bg-purple-50/20 h-full">
                                {UNITS.map(u => <option key={u} value={u}>{u}</option>)}
                            </select>
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="font-bold text-gray-700 text-sm flex items-center gap-2"><AlertTriangle size={16} className="text-purple-500"/> Low Stock Alert</label>
                        <Input ref={editLowStockRef} onKeyDown={(e) => handleEditorKeyDown(e, editMfgRef)} type="number" placeholder="10" value={newProduct.lowStockThreshold || ''} onChange={e => setNewProduct({...newProduct, lowStockThreshold: parseInt(e.target.value) || 0})} className="w-full border-2 border-purple-200 focus:border-purple-500 bg-purple-50/10 rounded-lg !py-3 !px-4" />
                    </div>
                    <div className="space-y-2">
                        <label className="font-bold text-gray-700 text-sm flex items-center gap-2"><Factory size={16} className="text-amber-500"/> Manufacturing Date</label>
                        <Input ref={editMfgRef} onKeyDown={(e) => handleEditorKeyDown(e, null, handleSaveProduct)} type="date" value={newProduct.manufacturingDate || ''} onChange={(e) => setNewProduct({...newProduct, manufacturingDate: e.target.value})} className="w-full border-2 border-amber-200 focus:border-amber-500 bg-amber-50/10 rounded-lg !py-3 !px-4" />
                    </div>
                 </div>
             )}
             <div className="grid grid-cols-3 gap-2 mt-8 pt-6 border-t border-gray-100">
                <Button variant="neutral" onClick={() => setIsEditorOpen(false)} className="w-full py-4 font-bold rounded-xl flex items-center justify-center px-1"><span className="truncate">Cancel</span></Button>
                <Button variant="neutral" onClick={handleAddToBatch} className="w-full py-4 font-bold rounded-xl bg-indigo-50 text-indigo-700 border-indigo-100 hover:bg-indigo-100 flex items-center justify-center gap-1 px-1"><ListPlus size={18} strokeWidth={2.5} className="shrink-0" /><span className="truncate">Queue</span></Button>
                <Button onClick={handleSaveProduct} className="w-full py-4 font-bold rounded-xl shadow-lg flex items-center justify-center gap-1 px-1">{isEditing ? <Save size={18} className="shrink-0"/> : (pendingBulkItems.length > 0 ? <Check size={18} strokeWidth={3} className="shrink-0"/> : <Plus size={18} strokeWidth={3} className="shrink-0"/>)}<span className="truncate">{isEditing ? "Update" : (pendingBulkItems.length > 0 ? "All" : "Save")}</span></Button>
             </div>
        </Card>
    </div>
  );
            ...
            <section className="mb-8 last:mb-0">
                {sectionTitle && (
                    <div className="flex items-center gap-3 mb-4 px-1 sticky top-0 bg-[#fdfdfc] z-10 py-2">
                        ...
                        <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">{sectionIcon} {sectionTitle}</h3>
    ...
    <div className="space-y-6 animate-in fade-in pb-24 relative">
        ...
        <div className="flex flex-col items-center gap-4 pt-2">
            <div className="relative w-full max-w-2xl mx-auto group">
                ...
                <div className="relative flex items-center bg-white rounded-full shadow-[0_2px_15px_-3px_rgba(0,0,0,0.1),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_20px_-2px_rgba(0,0,0,0.15)] transition-shadow duration-300 border border-gray-100">
                    <div className="pl-6 text-gray-400"><Search size={22} /></div>
                    <input type="text" placeholder="Search your inventory..." className="w-full bg-transparent border-none focus:ring-0 px-4 py-4 text-lg text-gray-800 placeholder-gray-400" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    {searchTerm && <button onClick={() => setSearchTerm('')} className="p-2 mr-1 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-all" title="Clear search"><X size={20} /></button>}
                    {(settings.scannerPreference === 'phone' || settings.scannerPreference === 'both') && <button onClick={() => setShowScanner(true)} className="pr-6 pl-2 text-gray-400 hover:text-blue-600 transition-colors tooltip" title="Scan Barcode"><Scan size={22} /></button>}
                </div>
            </div>
            <div className="w-full flex flex-col md:flex-row justify-between items-center gap-4 px-1">
                <div className="w-full md:flex-1 overflow-x-auto pb-2 scrollbar-hide flex items-center gap-2">
                     {[ { id: ProductFilter.ALL, label: 'All Stocks' }, { id: ProductFilter.LOW_STOCK, label: 'Low Stock' }, { id: ProductFilter.OUT_OF_STOCK, label: 'Out of Stock' }, { id: ProductFilter.EXPIRING_SOON, label: 'Expiring' } ].map(filter => (
                        <button key={filter.id} onClick={() => setActiveFilter(filter.id)} className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-bold transition-all border shrink-0 ${activeFilter === filter.id ? 'bg-gray-800 text-white border-gray-800 shadow-md' : 'bg-white border-gray-200 text-gray-500 hover:border-gray-300 hover:bg-gray-50'}`}>{filter.label}</button>
                    ))}
                </div>
            </div>
        </div>
        {isSearching && groupedSearchResults.length > 0 && (
             <div className="animate-in fade-in slide-in-from-top-4 duration-300 mb-8 p-6 bg-blue-50/50 border border-blue-100 rounded-2xl shadow-sm">
                <h3 className="text-sm font-bold text-blue-600 mb-4 uppercase tracking-wider flex items-center gap-2"><Search size={16} /> Top Match</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {groupedProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
                <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-4"><Search size={32} className="opacity-20 text-gray-500"/></div>
                <h3 className="text-lg font-bold text-gray-600">No products found</h3>
                {(activeFilter !== ProductFilter.ALL || searchTerm) && <Button variant="neutral" className="mt-6" onClick={() => { setActiveFilter(ProductFilter.ALL); setSearchTerm(''); }}>Clear all filters</Button>}
            </div>
        ) : (
  ...
  const renderTags = () => (
    <div className="space-y-6 animate-in fade-in max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <Button onClick={() => { setIsEditingTag(false); setNewTag({ name: '', color: '#3b82f6' }); setShowTagModal(true); }} className="rounded-full px-6 py-2.5 shadow-md whitespace-nowrap flex items-center"><Plus size={18} className="mr-2"/> Create New Tag</Button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tags.map(t => (
                <Card key={t.id} className="flex justify-between items-center group hover:shadow-lg transition-all border border-gray-100 cursor-pointer active:scale-95 relative" onClick={() => { setActiveTab(SubTab.PRODUCTS); }}>
                    <div className="flex items-center gap-4 flex-1">
                        <div className="w-12 h-12 rounded-full flex items-center justify-center bg-gray-50 border border-gray-100"><div className="w-4 h-4 rounded-full" style={{ backgroundColor: t.color }}></div></div>
                        <div><div className="font-bold text-gray-800 text-lg">{t.name}</div><div className="text-xs text-gray-500 font-medium">{products.filter(p => p.tagId === t.id).length} products</div></div>
                    </div>
        ...
        {tags.length === 0 && <div className="text-center py-20 text-gray-400"><TagIcon size={48} className="mx-auto mb-2 opacity-20"/><p>No categories found. Create one to organize your products.</p></div>}
    </div>
  );
  ...
  return (
    <div className="pb-32 min-h-[80vh]" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
      ...
      {!isEditorOpen && (
        <div className="flex justify-center mb-6 sticky top-4 z-30">
            ...
            {[ { id: SubTab.PRODUCTS, icon: Box, label: 'Products' }, { id: SubTab.TAGS, icon: TagIcon, label: 'Tags' }, { id: SubTab.SETTINGS, icon: Settings, label: 'Settings' } ].map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`relative flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${activeTab === tab.id ? 'bg-gray-900 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100/80 hover:text-gray-900'} ${tab.id !== SubTab.SETTINGS ? 'px-4' : 'px-2'}`} title={tab.label}><tab.icon size={18} />{tab.id !== SubTab.SETTINGS && <span className="hidden sm:inline">{tab.label}</span>}</button>
            ))}
            </nav>
        </div>
      )}
      {!isEditorOpen && activeTab === SubTab.PRODUCTS && (
          <div className="fixed bottom-24 right-4 z-40 flex flex-col gap-4">
              <button onClick={handleAnalyzeClick} disabled={isParsingInvoice} className="w-12 h-12 rounded-2xl bg-white/40 backdrop-blur-xl border border-white/60 text-indigo-700 shadow-lg flex items-center justify-center transition-all active:scale-95 hover:bg-white/60">{isParsingInvoice ? <Loader2 size={20} className="animate-spin"/> : <Sparkles size={20} />}</button>
              <button onClick={handleOpenAdd} className="w-12 h-12 rounded-2xl bg-white/40 backdrop-blur-xl border border-white/60 text-blue-700 shadow-lg flex items-center justify-center transition-all active:scale-95 hover:bg-white/60"><Plus size={24}/></button>
          </div>
      )}
      {isEditorOpen ? renderEditor() : (<main className="w-full mx-auto px-2">{activeTab === SubTab.PRODUCTS && renderProducts()}{activeTab === SubTab.TAGS && renderTags()}{activeTab === SubTab.SETTINGS && renderSettings()}</main>)}
      <Modal isOpen={showTagModal} onClose={() => { setShowTagModal(false); setIsSavingTag(false); setIsEditingTag(false); }} title={isEditingTag ? "Edit Category" : "Create New Tag"}>
          <div className="space-y-4">
              <div><label className="text-xs font-bold text-gray-500 uppercase block mb-1">Category Name</label><Input placeholder="e.g. Dairy" value={newTag.name || ''} onChange={e => setNewTag({...newTag, name: e.target.value})} onKeyDown={(e) => { if(e.key === 'Enter') handleSaveTag(); }} autoFocus/></div>
              ...
              <div>
                  ...
                  <div className="flex items-center gap-3">
                      <div 
                          className="w-10 h-10 rounded-lg shadow-sm border border-gray-200 shrink-0" 
      ...
      <Modal isOpen={showScanner} onClose={() => setShowScanner(false)} title="Scan Barcode"><div className="relative w-full bg-black rounded-xl overflow-hidden min-h-[300px]"><div id="reader" className="w-full"></div></div>{isScanningToAdd && <div className="mt-4 pt-4 border-t border-gray-100 flex justify-center"><Button variant="neutral" onClick={handleManualEntry} className="w-full">Can't scan? Enter Manually</Button></div>}</Modal>
      <Modal isOpen={!!itemToDelete} onClose={() => setItemToDelete(null)} title="Confirm Delete"><div className="mb-6"><p className="text-gray-600 mb-2">Delete <strong>{itemToDelete?.name}</strong>?</p>{itemToDelete?.type === 'tag' ? <div className="bg-blue-50 p-3 rounded-lg flex gap-3 text-sm text-blue-800"><AlertTriangle size={16} />Products will NOT be deleted.</div> : <p className="text-sm text-red-500">This action cannot be undone.</p></div><div className="flex gap-4 justify-end"><Button variant="neutral" onClick={() => setItemToDelete(null)}>Cancel</Button><Button variant="danger" onClick={confirmDelete}>Delete</Button></div></Modal>
      <Modal isOpen={showSourceOptions} onClose={() => setShowSourceOptions(false)} title="Analyze Image"><div className="space-y-3"><button onClick={handleCameraOption} className="w-full flex items-center gap-3 p-4 rounded-xl border border-gray-200 hover:border-blue-500 hover:bg-blue-50 transition-all"><div className="p-3 rounded-full bg-blue-100 text-blue-600"><Camera size={24}/></div><div className="text-left"><span className="font-bold text-gray-900 block">Take Photo</span><span className="text-xs text-gray-500">Capture invoices</span></div></button><button onClick={handleUploadOption} className="w-full flex items-center gap-3 p-4 rounded-xl border border-gray-200 hover:border-purple-500 hover:bg-purple-50 transition-all"><div className="p-3 rounded-full bg-purple-100 text-purple-600"><FileType size={24}/></div><div className="text-left"><span className="font-bold text-gray-900 block">Upload File</span><span className="text-xs text-gray-500">Image or PDF</span></div></button></div></Modal>
      {showCamera && (
          <div className="fixed inset-0 z-[100] bg-black flex flex-col"><div className="relative flex-1 bg-black"><video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover"></video></div><div className="h-32 bg-black flex items-center justify-around px-8 pb-8 pt-4"><button onClick={() => setShowCamera(false)} className="p-4 rounded-full bg-white/10 text-white"><X size={24}/></button><button onClick={capturePhoto} className="w-20 h-20 border-4 border-white rounded-full flex items-center justify-center"><div className="w-full h-full bg-white rounded-full"></div></button><div className="w-14"></div></div></div>
      )}
    </div>
  );
};
