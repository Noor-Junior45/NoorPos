...
export const Warehouse: React.FC<WarehouseProps> = ({ initialAction, onClearAction }) => {
  ...
  // Fixed error in file src/pages/Warehouse.tsx: added missing 'const' keyword and moved it up to be defined before its first usage in capturePhoto.
  ...
  const handleSaveProduct = async () => { 
      // CASE 1: There is a queue
      if (pendingBulkItems.length > 0) {
          const itemsToSave = [...pendingBulkItems];
          
          // Check if current form has a name or price (meaning user might be in middle of entry)
          const hasUnfinishedEntry = newProduct.name?.trim() || (newProduct.sellPrice && newProduct.sellPrice > 0);
          
          if (hasUnfinishedEntry) {
              if (!validateProduct()) return; // Stop if the user is in middle of entry but it's invalid
      ...
      // CASE 2: Single item save (standard)
      if (!validateProduct()) return;
                  ...
                  <div className="flex items-center gap-3">
                      ...
                      <Input 
                          ...
                          onChange={e => {
                              ...
                              // Basic validation: allow # and hex chars
