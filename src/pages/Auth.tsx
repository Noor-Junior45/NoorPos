  ...
  return (
    <div className="min-h-screen flex flex-col justify-center bg-[#f8fafc] p-6">
        <div className="w-full max-w-[420px] mx-auto animate-in fade-in zoom-in duration-500">
            ...
            <div className="space-y-4">
              ...
              {error && (
                  <div className="p-3 rounded-xl bg-red-50 text-red-600 text-sm font-medium text-center border border-red-100 flex flex-col items-center gap-2 justify-center">
                      ...
                      {apiErrorLink && <a href={apiErrorLink} target="_blank" rel="noopener noreferrer" className="mt-1 flex items-center gap-1 text-xs font-bold text-red-700 underline hover:text-red-900 bg-red-100 px-3 py-1.5 rounded-lg"><ExternalLink size={12} /> Enable Google Sheets API</a>}
                  </div>
              )}
            </div>
          </div>
          ...
          <div className="text-center mt-4 text-[10px] font-medium text-gray-400">Noor POS System v1.6</div>
        </div>
    </div>
  );
};
